...
import CommProcedureField from './CommProcedureField';

const CommProcedureForm = ({ mode = 'ADD', param }) => {
  ...
  useEffect(() => {
    if (mode === 'EDIT' && param?.id) {
      const fetchDocument = async () => {
        const docRef = doc(db, 'commProcedure', param.id);
        ...
        if (docSnap.exists()) {
          ...
          navigate('/app/apps/comm-procedure/comm-procedure-list');
        }
      };
      fetchDocument();
    }
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (mode === 'ADD') {
        await addDoc(collection(db, 'commProcedure'), {
          ...docData,
          createdAt: new Date(),
        });
      ...
      form.resetFields();
      navigate(`/app/apps/comms-procedure/comm-procedure-list`);
    } catch (error) {
...
export default CommProcedureForm;
